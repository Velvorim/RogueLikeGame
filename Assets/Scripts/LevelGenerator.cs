...
public class LevelGenerator : MonoBehaviour
{
    ...
    public enum Direction { up, rigth, down, left };
    ...
    public RoomCenter[] potentialsCenters;

    // Start is called before the first frame update
    void Start()
    {
        //this will color the start room
        Instantiate(layoutRoom, generatorPoint.position, generatorPoint.rotation).GetComponent<SpriteRenderer>().color = startColor;

        selectedDirection = (Direction)Random.Range(0, 4);
        MoveGenerationPoint();

        //Generate randomic room 
        for (int i = 0; i < distanceToEnd; i++)
        {
            ...
            //so here we have this function(Phiscs2D.OverlapCicle(here the circle will generate, the size of the cirle in radius, and the layer that he will interact)
            //this is made because the room are overlapsing each other, and this while whit this function gone salve us 
    ...
    void Update()
    {
#if UNITY_EDITOR // this lin and make the code in the scope of the if only run in the unity editor, very nice for test and somthing like that
    ...
    public void CreateRoomOutline(Vector3 roomPosition)
    {
        ...
        switch (directionCount)
        {
            ...
            case 2:
                ...
                {
                    genaratedOutlines.Add(Instantiate(rooms.doubleRigthDown, roomPosition, transform.rotation));
...
public class RoomsPrefabs
{
    public GameObject singleUp, singleDown, singleRight, singleLeft,
        doubleUpDown, doubleLeftRight, doubleUpRight, doubleRigthDown, doubleDownLeft, doubleLeftUp,
